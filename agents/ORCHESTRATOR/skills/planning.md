...
2. Store the full plan in your KV store using `plan_save()`.
...
Use `plan_save("plan:<plan_id>", json.dumps(plan))` to store and
`json.loads(plan_load("plan:<plan_id>"))` to retrieve.

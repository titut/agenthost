...
5. **Use clean Markdown formatting.** Put a blank line before and after every header, list, and table. Each bullet or numbered item must be on its own line. Do not run list items together on the same line.
6. **Keep the plan straightforward.** Each step should be a single prompt string. One thread has one plan. Replace the whole plan with `plan(steps=[...])` when the approach changes.

...
10. **Pass plan fields as separate arguments.** When calling `plan_save`, pass `plan_id`, `status`, `request`, `coarse_steps`, and `fine_steps` as separate parameters. Do not wrap the whole plan in a JSON string and do not use a single `plan` object parameter.
